        ...
        <Row className="p-2 p-sm-4 g-4">
          <Col md={6} xl={3}>
            <ul className="list-unstyled">
              ...
              {MEGA_MENU_ITEMS.usefulLinks.map((item, idx) => (
                <li key={idx}>
                  <DropdownItem as={Link} className="dropdown-item" to="">
                    {item}
                  </DropdownItem>
                </li>
              ))}
          ...
          <Col md={6} xl={3}>
            ...
            {MEGA_MENU_ITEMS.company.map((item, idx) => {
              ...
              return (
                <div
                  className="dropdown-item d-flex bg-light-hover position-relative text-wrap py-3"
                  key={idx}
                >
            ...
            <ul className="list-unstyled">
              ...
              <li>
                ...
                {MEGA_MENU_ITEMS.integration.map((item, idx) => (
                  <Link key={idx} className="dropdown-item d-flex bg-light-hover icon-link icon-link-hover py-2" to="">
                    <img src={item.logo} className="h-30px" alt="" />
                    <span className="heading-color fw-bold ms-2">
                      {item.name}
                    </span>
                ...
                <Link to="" className="btn btn-sm btn-light mb-0 d-grid mt-2">
                  View all
                </Link>
              </li>
            </ul>
          ...
          <Col md={6} xl={3}>
            ...
            {MEGA_MENU_ITEMS.recentBlogs.map((item, idx) => (
              <div key={idx} className="dropdown-item bg-light-hover d-flex align-items-sm-center gap-2 position-relative mb-3">
                <img src={item.image} className="rounded icon-lg" alt="blog-img" />
                <p className="text-wrap fw-bold mb-0">
                  <Link to={item.link} className="stretched-link heading-color text-primary-hover">
                    {item.title}
                  </Link>
                </p>
              </div>
