  ...
  return (
    <Card className="card-body bg-light d-md-flex justify-content-md-between align-items-md-center flex-md-row p-4 mb-3">
      <div className="mb-4 mb-md-0">
        ...
        <h5 className="mb-0"><Link to={`/career/single?title=${name}`} className="stretched-link">{name}</Link></h5>
        <div className="hstack gap-3 gap-sm-4 flex-wrap mt-3">
          <span><BsGeoAlt className="me-2" />{address}</span>
          <span><BsClock className="me-2" />{time}</span>
          <span><BsBriefcase className="me-2" />{currency}{salary}</span>
        </div>
      </div>
      <div><Link to={`/career/single?title=${name}`} className="btn btn-primary mb-0">Apply now<BsArrowRight className="ms-2" /></Link></div>
    </Card>
  )
}
    ...
    <section className="pt-0">
      <Container>
        <div className="inner-container-small text-center mb-4 mb-sm-6">
          ...
          <p className="mb-0">Our friendly team members are always willing to help you understand your present technology requirements and provide suggestions on your future needs.</p>
        </div>
              ...
              <TabContent>
                ...
                <TabPane eventKey={"3"} className="fade" >
                  <div className="text-center justify-content-center p-sm-4">
                    <span className="display-5 text-primary"><BsSlashCircle /></span>
