...
import {pricingData1, pricingData2} from "@/pages/saas/V2/data";
import PricingCard1 from "@/pages/saas/V2/components/PricingCard1";
import PricingCard2 from "@/pages/saas/V2/components/PricingCard2";
            ...
            <Container className="position-relative">
                <Row className="g-4 g-lg-6">
                    ...
                    <Col xl={4}>
                        ...
                        <p className="mb-5">Our pricing model is designed with your agency in mind. Discover our
                            transparent rates and flexible options tailored to fit your unique needs.</p>
                        ...
                        <Form className="d-flex align-items-center">
                            ...
                            <div className="form-switch form-check-lg mx-2 mb-0">
                                <Form.Check type='switch' className="mt-0 price-toggle" defaultChecked={false}
                            ...
                            pricingData1.map((item) => (
                                item.plans.map((plan, idx) => {
                                    if (item.duration === planDuration)
                                        return (
                                            <PricingCard1 plan={plan} duration={item.duration} key={idx}/>
                                        )
                                })
                            ))
                        }
                    ...
                    <Col md={6} xl={4}>
                        {
                            pricingData2.map((item) => (
                                item.plans.map((plan, idx) => {
                                    if (item.duration === planDuration)
                                        return (
                                            <PricingCard2 plan={plan} duration={item.duration} key={idx}/>
                                        )
                                })
                            ))
                        }
