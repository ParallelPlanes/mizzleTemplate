import {TestimonialType} from "@/pages/saas/V2/types";
...
const TestimonialCard = ({testimonial}: TestimonialCardPropsType) => {
    return (
        <Card className="bg-transparent p-0 pe-md-7">
            ...
            <img src={testimonial.user.avatar} className="rounded"
                 alt="review-img"/>

            <Col xl={9} xs={10}
                 className="card-body bg-body shadow rounded position-absolute end-0 bottom-0 p-xl-5 mb-4 me-4">

                <h6>{testimonial.title}</h6>
                <p>{testimonial.comment}</p>

                {
                    testimonial.rating && <ul className="list-inline mb-1">
                        {
                        ...
                        }
                        {
                            ...
                            <BsStarHalf size={16} className="text-primary me-1"/>
                        }
                        {
                ...
                <span className="heading-color fw-bold">{testimonial.user.firstName} {testimonial.user.lastName}</span>
