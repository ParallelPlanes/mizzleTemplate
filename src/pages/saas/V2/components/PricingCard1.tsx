...
import {PricingPlanType, PricingType} from "@/pages/saas/V2/types";
...
const PricingCard1 = ({plan, duration}: PricingCard1PropsType) => {
    return (
        <Card className="card-body bg-light p-4 p-sm-5">
            ...
            <figure
                className="position-absolute top-0 start-0 translate-middle mt-n4 ms-n5 d-none d-md-block">
                <svg className="fill-mode" width="86" height="105"
                     viewBox="0 0 86 105"
                     fill="none"
            ...
            <p className="heading-color fw-normal mb-1">How many customers do you
                have?</p>

            <Form className="mb-4">
                <Form.Control size='lg'
                              className="focus-shadow-none bg-transparent border-0 border-bottom rounded-0 ps-0"
                              type="email" placeholder="000"/>
            </Form>


            <span className="mb-4 heading-color">
                <span className="h2 plan-price">{currency}{plan.price}</span> /{duration}</span>

            <Link className="btn btn-primary icon-link icon-link-hover justify-content-center"
               to="">
                Get started<BsArrowRight className="bi"/>
            </Link>
        </Card>
    )
}
