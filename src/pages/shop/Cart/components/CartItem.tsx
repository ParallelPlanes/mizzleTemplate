      ...
      <Row className="align-items-xl-center">
        ...
        <Col xs={7} md={10}>
          <Row className="g-3 g-sm-4">
            <Col xl={5}>
              <h6 className="mb-1"><Link to="">{product.name}</Link></h6>
              {product.attributes && <ul className="nav nav-divider small align-items-center mt-1">
            ...
            <Col sm={6} md={4} xl={3} className="text-md-center">
              {product.sale ? <h5 className="mb-0">{currency}{getDiscountedPrice(product.price, product.sale.discount, product.sale.type)}</h5>
                :
                <h5 className="mb-0">{currency}{product.price.toFixed(2)}</h5>}
            </Col>
            <Col sm={6} md={4} xl={2} className="text-md-center">
              <Link to="" className="text-danger small icons-center gap-1" onClick={() => updateQuantityForProduct(product, 0)}><BsXLg /> Remove</Link>
            </Col>
          </Row>
        </Col>
      </Row>
