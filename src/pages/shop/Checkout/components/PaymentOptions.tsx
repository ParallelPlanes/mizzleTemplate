...
import { Card, CardBody, CardHeader, Col, Form, FormCheck, FormControl, FormLabel, Modal, ModalBody, ModalFooter, ModalHeader, Row } from "react-bootstrap"
import { BsPencilSquare, BsXLg } from "react-icons/bs"
import { Link } from "react-router-dom"

import { paymentOptions } from "@/pages/shop/Checkout/data"
    ...
    <>
                  ...
                  <div className="form-check border rounded p-3">
                    <div className="d-flex justify-content-between">
                      <label className="form-check-label d-flex align-items-start pe-4" htmlFor={`standard-${idx}`}>
                        {item.image && <img src={item.image} className="w-40px me-3 mt-1" />}
                        {Icon && <span className="w-40px"><Icon className="fs-5" /></span>}
                        <span>
                          <span className="d-block fw-semibold heading-color mb-1">{item.title}</span>
                          <span className="d-block small">{item.subTitle}</span>
                        </span>
                      </label>
                      <input defaultChecked={idx == 0 ? true : false} className="form-check-input" type="radio" name="payOptions" id={`standard-${idx}`} />
                    </div>
                    <div className="d-flex justify-content-end gap-3 mt-3">
                      <Link to="" className="text-body-secondary text-primary-hover small">Set as default</Link>
                      {item.image &&
                        <Link to="" onClick={toggle} className="heading-color text-primary-hover fw-semibold small" data-bs-toggle="modal" data-bs-target="#editcard"><BsPencilSquare className="me-1" />Edit</Link>
                      }
                    </div>
                  </div>
                </Col>
      ...
      <Modal show={isTrue} onHide={toggle} centered className="fade" id="editcard" tabIndex={-1} aria-labelledby="editCardLabel" aria-hidden="true">
        <ModalHeader className="px-4 d-flex justify-content-between">
          <h5 className="modal-title" id="editCardLabel">Edit card</h5>
          <button type="button" className="btn btn-sm btn-light mb-0" onClick={toggle}><BsXLg /></button>
        </ModalHeader>
          ...
          <Row as={Form} className="g-4">
            <Col xs={12}>
              ...
              <div className="position-relative">
                ...
                <img src={visa} className="w-40px position-absolute top-50 end-0 translate-middle-y me-2" />
              </div>
            </Col>
            <Col md={6}>
        ...
        <ModalFooter className="px-4">
          <button type="button" className="btn btn-white border my-0" onClick={toggle}>Close</button>
          <button type="button" className="btn btn-primary my-0" data-bs-dismiss="modal">Save change</button>
        </ModalFooter>
      </Modal>
