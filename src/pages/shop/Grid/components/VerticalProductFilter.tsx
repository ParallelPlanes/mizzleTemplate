...
import { categories } from "@/pages/shop/Grid/data"
          ...
          <AccordionBody as={"form"}>
            {categories.map((category, idx) => (
              <div className={`form-check ${idx === 0 ? "" : "mt-2"}`} key={idx}>
                ...
                <label className="form-check-label heading-color" htmlFor={`category${idx}`}>{category.name}</label>
        ...
        <AccordionItem eventKey="2">
          ...
          <AccordionBody as={"form"}>
            <div className="form-check">
              ...
              <label className="form-check-label heading-color" htmlFor="price1">{currency}0 to {currency}200</label>
            </div>

            <div className="form-check mt-2">
              <input className="form-check-input" type="checkbox" id="price2" />
              <label className="form-check-label heading-color" htmlFor="price2">{currency}200 to {currency}500</label>
            </div>

            <div className="form-check mt-2">
              <input className="form-check-input" type="checkbox" id="price3" />
              <label className="form-check-label heading-color" htmlFor="price3">{currency}500 to {currency}1500</label>
            </div>

            <div className="form-check mt-2">
              <input className="form-check-input" type="checkbox" id="price4" />
              <label className="form-check-label heading-color" htmlFor="price4">{currency}1500 to more</label>
            </div>
          </AccordionBody>
        </AccordionItem>
        ...
        <AccordionItem eventKey="4">
          ...
          <AccordionBody as={'form'}>
            <ul className="list-inline mb-0 g-3">
              {Array(5).fill(0).map((_star, idx) => (
                <li className="list-inline-item mb-0" key={idx}>
                  <input type="checkbox" className="btn-check" id={`btn-star-${idx}`} />
                  <label className="btn btn-sm btn-light btn-primary-soft-check icons-center" htmlFor={`btn-star-${idx}`}>{idx + 1}<BsStarFill /></label>
                </li>
              ))}
