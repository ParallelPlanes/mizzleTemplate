        ...
        <section className="pt-8">
          <Container>
                ...
                <ul className="nav nav-pills nav-pills-primary-soft gap-2">
                  ...
                  {categoriesData.map((category, idx) => (
                    <li key={idx} className="nav-item">
                      <a className={`nav-link ${idx === 0 ? 'active' : ''}`} data-filter={category.name}>{category.title}</a>
                    </li>
                  ))}
                ...
                <div key={idx} className={`col-sm-6 col-lg-4 grid-item ${item.categories.map((category) => ' ' + category + ' ')}`}>
                  <Card className="card-element-hover card-metro-hover">
                    ...
                    <div className="card-img-overlay hover-element d-flex">
                      <CardText as={"p"} className="mt-auto">
                        <h6 className="mb-0"><Link to="/portfolio/case-study/v1" className="text-white stretched-link">{item.title}</Link></h6>
            ...
            <div className="text-center mt-7">
              <button className="btn btn-lg btn-outline-light mb-0 icons-center"><FaSpinner size={20} className="me-2" />Load more work</button>
            </div>
          </Container>
        </section>
      </main>
